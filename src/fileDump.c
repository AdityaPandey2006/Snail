...
int cleanDump(){
    ...
    while((entry=readdir(dumpFileDir))!=NULL){
        ...
        if(currentTime-deletionTime>(48*60*60)){
            ...
            remove(entryFilePath);//will fail for directories. will have to use recursive deletion here
        }       
    }
    closedir(dumpDir);
...
int fileRestoreHelper(char* entryPath){
    ...
    DIR* dumpDir=opendir(dumpPath);
    DIR* dumpFilesDir=readdir(dumpFilesPath);
    DIR* dumpInfoDir=readdir(dumpInfoPath);
    struct dirent* entry;
    while((entry=readdir(dumpDir))!=NULL){
        char entryName[PATH_MAX + 1];
        if(strcmp(entry->d_name,".")==0||strcmp(entry->d_name,"..")==0){
            continue;
        }
        ...
        while(fgets(buffer,maxLineLength,entryInfoFilePointer)!=NULL){
            if(pFound){
                int newLinePos=strcspn(buffer,"\n");
                buffer[newLinePos]='\0';
                strcpy(originalPath,buffer);
            }
            else{
                if(strncmp(buffer,"Path:",13)==0){
                    pFound=1;
                }
            }
        }
        int originalPathSize=strlen(originalPath);
        if(originalPath[originalPathSize-1]=='/'){
            originalPath[originalPathSize-1]='\0';
        }
        if(rename(entryFilePath,originalPath)!=0){
            success=0;
            return success;
        }
        remove(entryInfoPath);
